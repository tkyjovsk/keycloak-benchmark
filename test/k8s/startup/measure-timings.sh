...
until [ "$(kubectl -n ${NAMESPACE} get keycloak/keycloak -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' | tr '[:upper:]' '[:lower:]')" == "true" ]; do sleep 1; done
...
for pod in $(kubectl -n ${NAMESPACE} get pods -o name | grep -oP "keycloak-[0-9]+"); do 
  ...
  kubectl -n ${NAMESPACE} wait --for=condition=PodScheduled=true pod/${pod}
  podUID=$(kubectl -n ${NAMESPACE} get pods/${pod} -o jsonpath='{.metadata.uid}')
  if grep -q "$podUID" "$csvFile"; then
    echo "Timing information for pod '$pod' with UID '$podUID' already recorded in '$csvFile'. Skipping."
  else

    # Pod timings
    kubectl -n ${NAMESPACE} wait --for=condition=Initialized=true pod/${pod}
    kubectl -n ${NAMESPACE} wait --for=condition=ContainersReady=true pod/${pod}
    kubectl -n ${NAMESPACE} wait --for=condition=Ready=true pod/${pod}

    timePodScheduled=$(kubectl -n ${NAMESPACE}  get pods/${pod} -o jsonpath='{.status.conditions[?(@.type=="PodScheduled")].lastTransitionTime}')
    timeInitialized=$(kubectl -n ${NAMESPACE}  get pods/${pod} -o jsonpath='{.status.conditions[?(@.type=="Initialized")].lastTransitionTime}')
    timeContainersReady=$(kubectl -n ${NAMESPACE}  get pods/${pod} -o jsonpath='{.status.conditions[?(@.type=="ContainersReady")].lastTransitionTime}')
    timeReady=$(kubectl -n ${NAMESPACE}  get pods/${pod} -o jsonpath='{.status.conditions[?(@.type=="Ready")].lastTransitionTime}')
    ...
    keycloakAugmentationMillis=$(kubectl -n ${NAMESPACE} logs $pod | grep 'io.quarkus.deployment.QuarkusAugmentor' | grep -oP 'Quarkus augmentation completed in \K[0-9]+' | tail -n 1)
    ...
    keycloakStartedInSeconds=$(kubectl -n ${NAMESPACE} logs $pod | grep 'io.quarkus' | grep Keycloak | grep -oP 'started in \K[0-9\.]+' | tail -n 1)
