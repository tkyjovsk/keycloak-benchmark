...
function measurePodMemory {
  ...
  until ! kubectl -n ${NAMESPACE} get pods/${pod} | grep -i Terminating && [ "$(kubectl -n ${NAMESPACE} get pods/${pod} -o jsonpath='{.status.conditions[?(@.type=="Initialized")].status}' | tr '[:upper:]' '[:lower:]')" == "true" ]; do
  ...
  for measurementTime in $measurementTimes; do
    ...
    if [ ${podRemainingMeasurementTime} -lt 0 ]; then
      ...
      memoryUsageInBytes=$(kubectl -n ${NAMESPACE} exec ${pod} -- cat /sys/fs/cgroup/memory.current)
      rssInBytes=$(kubectl -n ${NAMESPACE} exec keycloak-0 -- cat /proc/1/smaps | grep -i rss | awk '{sum+=$2} END {print (sum*1024)}')
